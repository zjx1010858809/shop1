...
public interface orders_details_dao {
	...
	@Select("select od.*,o.date date,o.code code,p.fullname,p.pics pics,o.status status from orders_details od inner join orders o inner join product p on od.orders_id=o.id and od.product_id=p.id where o.user_id=#{user_id} order by od.id desc")
